...
def augmentations(image):
    ...
    image = image.rotate(random.uniform(-10, 10), resample=Image.BICUBIC, fillcolor=255)
    
    arr = np.array(image)
    noise_level = random.choice([0.01, 0.02, 0.03])
    noise = np.random.randint(0, 40, arr.shape) * (np.random.rand(*arr.shape) < noise_level)
...
def generate_digit_image(digit, system="western", augment=True):
    ...
    if system == "western":
        ...
        roman_digits = ["N", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX"]
    ...
    try:
        ...
        font = ImageFont.truetype(random.choice(["arial.ttf", "times.ttf", "verdana.ttf"]), font_size)
    except IOError:
...
def generate_separate_datasets(output_dir="digit_dataset", samples_per_digit=500, test_size=0.2, mislabel_hard_fraction=0.5, save_images=False):
    ...
    for system in systems:
        ...
        print(f"--- Генерация датасета для {system.replace('_', ' ').title()} чисел ---")
        for digit in range(10):
            for _ in range(normal_samples):
                img_array, _, img_obj = generate_digit_image(digit, system=system)
            ...
            for _ in range(hard_samples):
                ...
                if should_mislabel:
                    mislabel_digit = random.choice([d for d in range(10) if d != digit])
                    img_array, _, img_obj_base = generate_digit_image(mislabel_digit, system=system)
                    true_label = digit
                else:
                    img_array, _, img_obj_base = generate_digit_image(digit, system=system)
                    true_label = digit

                img_obj_hard_augmented = img_obj_base

                if random.random() > 0.5:
                    img_obj_hard_augmented = img_obj_hard_augmented.filter(ImageFilter.GaussianBlur(radius=random.uniform(1.5, 2.5)))
                else:
                    arr = np.array(img_obj_hard_augmented)
                    noise = np.random.randint(0, 80, arr.shape) * (np.random.rand(*arr.shape) < 0.1)
                    img_obj_hard_augmented = Image.fromarray(np.clip(arr + noise, 0, 255).astype("uint8"))


                img_obj_hard_augmented = img_obj_hard_augmented.resize((28, 28), Image.LANCZOS)
                img_array_final = np.array(img_obj_hard_augmented, dtype=np.uint8) / 255.0
                
                X_all_data.append((img_array_final, img_obj_hard_augmented)) 
                y_all_labels.append(true_label)

            print(f"  [{system}] Цифра {digit}: сгенерировано {samples_per_digit} примеров.")
        ...
        if save_images:
            for idx, label in enumerate(y_train_np):
                ...
                img_filename = os.path.join(train_images_dir, f"{system}_{label}_{img_counter_train:05d}.png")
                img_to_save.save(img_filename)
                img_counter_train += 1

            for idx, label in enumerate(y_test_np):
                img_to_save = X_test_images[idx]
                img_filename = os.path.join(test_images_dir, f"{system}_{label}_{img_counter_test:05d}.png")
                img_to_save.save(img_filename)
                img_counter_test += 1
            print(f"  [{system.replace('_', ' ').title()}] Сохранено {img_counter_train} изображений в train и {img_counter_test} в test.")

        X_train_tensor = torch.tensor(X_train_np[:, None, :, :], dtype=torch.float32)
        y_train_tensor = torch.tensor(y_train_np, dtype=torch.long)
        X_test_tensor = torch.tensor(X_test_np[:, None, :, :], dtype=torch.float32)
        y_test_tensor = torch.tensor(y_test_np, dtype=torch.long)

        torch.save(X_train_tensor, os.path.join(output_dir, f"{system}_X_train.pt"))
        torch.save(y_train_tensor, os.path.join(output_dir, f"{system}_y_train.pt"))
        torch.save(X_test_tensor, os.path.join(output_dir, f"{system}_X_test.pt"))
        torch.save(y_test_tensor, os.path.join(output_dir, f"{system}_y_test.pt"))
